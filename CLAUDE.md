...
>
> ⚠️ **`manifest_refresh: agent-enumeration-only`.** Do **NOT** run `bin/write-manifest.mjs` on this plugin. Its tracer only sees direct calls, and this codebase registers through wrappers — it zeroes **shortcodes** (0 vs 17 real, registered via `src/Engine/ShortcodeHandler.php`), collapses **cron** (1 vs 16 — Action Scheduler + `self::CONST` hooks), and undercounts **REST** (62 vs 76 — routes built by concatenation, not literals). It will silently overwrite a good manifest with a worse one. If it is ever run: `git checkout -- audit/manifest.json` and enumerate by hand.
>
> **Latest QA baseline:** [`audit/wppqa-baseline-2026-07-12/SUMMARY.md`](audit/wppqa-baseline-2026-07-12/SUMMARY.md). php-lint 337/0, api 62/0, database 39/0, rest-js-contract 10/0 all clean. Open: a11y (18), admin-eval (28), enum-consistency (5), qa-coverage (1). **All 5 HIGH security findings are in bundled third-party libs** (`libs/woocommerce/action-scheduler`, `libs/easy-digital-downloads`) — our own code has zero. Do not "fix" vendored libs; the patch is lost on re-vendor.
>
> **Folder map:**
...
| Version | Key Changes |
|---|---|
| **1.6.4** (in dev, 2026-07-12, branch `1.6.4`) | **Notifications queue: one store, one reader, bounded on write** (Basecamp #10086171887). The queue had accreted two of everything and the halves disagreed: `push()` wrote a transient AND the durable table; the transient was capped at 50 by `array_slice` while the table **appended forever**; transient ids reset to 1 on every cache flush while the table's AUTO_INCREMENT never does; and a block in `push()` existed purely to drag new ids past a stale cursor — a hack whose only reason to exist was the transient's resetting ids. Reads had already moved table-first, so **the unbounded half was the one being read**. A load test left one member with 30,197 rows, replayed 50 toasts/page for ~600 page views. **Consolidated:** (1) **ONE store** — transient dual-write, its bound, its dead read branch and the id-reconciliation hack all deleted; (2) **ONE reader** — `NotificationBridge::fetch_unseen()`; footer/heartbeat/REST drive it via `read_pending()` with a user_meta cursor, SSE with a request cursor. `SSEController::fetch_pending()` held a **duplicate** `ORDER BY id ASC LIMIT 50` (worse there: polls every 2s for 28s ⇒ ~700 stale toasts/connection, then EventSource reconnects) — **deleted**; (3) **ONE bound** — `trim()` caps each member at `QUEUE_MAX_EVENTS` **on write**, so the table cannot grow without limit even on `DISABLE_WP_CRON` hosts. The prune is *retention*, not the bound; (4) reads are **burst-capped at `BURST_MAX_EVENTS` (5)** and **fast-forward the cursor to the head of the backlog** — a backlog is skipped, never replayed; (5) `prune_queue()` batches in a loop (5k × 20) — it was a single un-looped `LIMIT 5000` DELETE/day, so any site producing more than that per day fell permanently behind; (6) one-time **resumable purge** of legacy `type='skip'` rows (`DbUpgrader::ensure_notifications_skip_purge`); (7) an undecodable payload row now advances the cursor instead of **wedging a member's queue permanently**. <br>**Product: a member is never told they earned nothing.** Skip toasts shipped in 1.4.1 and were made opt-in in 1.6.3 — still wrong: an opt-in lever keeps a member-hostile surface alive, keeps writing rows, and invites an owner to switch a demotivator back on. **Mechanism removed**: no listener on `wb_gam_award_skipped`, `on_award_skipped()` and the `wb_gam_award_skip_toast_reasons` filter **deleted**. The copy is **relocated, not deleted** — right for an API caller, wrong for a toast: `PointsEngine::skip_reason_message()` (API/log only) + `POST /events` now returns `{skipped:{reason,message,context}}` alongside `processed:false` (previously a bare `false` with no way to tell "on cooldown" from "no such rule"). `wb_gam_award_skipped` is still fired and remains a supported extension point. <br>**Also:** EDD SL SDK asset 404 on symlinked/non-prefixing hosts (Basecamp #10081934029) — `Path::set()` built the URL by subtracting `$_SERVER['DOCUMENT_ROOT']`, which prefix-matches nothing when `__FILE__` resolves a symlink, leaking the full filesystem path into the URL. Now `plugins_url()`, kept as a marked `WBCOM PATCH` so it survives re-vendoring. **Note: the SDK is vendored under one shared namespace across many plugins, so `class Path` loads ONCE from whichever plugin wins the load race — one unpatched plugin defeats the fix in all the others.** <br>**Inventory delta**: hooks 141→142 (`wb_gam_email_footer`), cron 14→16 (`wb_gam_nudge_single_user`, `wb_gam_webhook_retry` — Action Scheduler singles). Suite 231/231, phpcs + phpstan clean. |
| **1.5.3** (in dev, 2026-06-02) | **Site-owner controls release** (branch `feat/site-owner-controls-1.5.3`). Adds the admin controls communities expect: (1) **Settings > Access** earning exclusion - `PointsEngine::user_can_earn()` + `excluded_user_ids()` (cached, flushable) + `wb_gam_user_can_earn` filter, enforced at the rate-limit gate AND directly in `award()`/`award_batch()` (login bonus, community-challenge bonus, approved submissions call `award()` directly and would otherwise bypass it); manual admin REST award (unregistered `manual_award`) + CLI `points award` (`force=true`) override; excluded users hidden from leaderboards via `get_opted_out_ids()`; `wb_gam_sandboxed` meta wired into Privacy export+erase. (2) **Members roster** admin page (`MembersPage` + `GET/POST wb-gamification/v1/members*`) - searchable/paginated, N+1-safe via prime_totals+prime_earned_badges, per-member award/exclude/reset. (3) **Bulk award** (`POST points/bulk` + Award Points UI) via exclusion-aware `award_batch`. (4) **Settings > Tools**: import/export config (`WBGam\Engine\SettingsIO` + `ToolsController`, excludes runtime/schema state) + Rebuild-leaderboard button. (5) **Settings > Modules** optional-module on/off toggles (`WBGam\Engine\ModuleToggles`) - disabled modules' blocks/shortcodes render nothing + admin page removed; data preserved. (6) **Settings > Points** opt-in inactivity point decay (`WBGam\Engine\PointsExpiry`, daily `wb_gam_points_decay` cron, OFF by default). (7) **Settings > Tools** reset-all-member-progress (`WBGam\Engine\ProgressReset` + `POST tools/reset-progress`, admin + explicit confirm; truncates progress tables, keeps config). **New engine classes**: SettingsIO, ModuleToggles, PointsExpiry, ProgressReset. **New REST controller**: ToolsController. **Inventory delta**: REST 56->64, admin pages 13->14, services 45->49, cron 10->11, filters 66->68, actions 58->60. **Shipped in two PRs**: #50 (controls 1-4 + recompute) merged to main; p2 branch `feat/owner-controls-p2-1.5.3` adds modules + expiry + reset. Tests: AccessExclusionTest, SettingsIOTest, ModuleTogglesTest, ProgressResetTest, PointsExpiryTest (163 unit tests green). All new endpoints admin-gated + namespaced (no WP/BP REST conflict). |
| **1.5.2** (2026-06-01) | **Performance + realtime-scale + member-surface + integration release.** **Realtime rescale for 100k users**: transport defaults to WP Heartbeat (15s steady / 5s burst-30s after an action / near-suspend on hidden tabs) instead of an SSE long-poll that pinned a PHP-FPM worker per logged-in page; SSE is now opt-in behind the `wb_gam_sse_allowed` filter. **N+1 elimination**: member directory 82→4 queries/page, leaderboard badges 20→1, top-members levels 42→0 on warm data, via `PointsEngine::prime_totals()` + `BadgeEngine::prime_earned_badges()`; validated against a 1M-row / 100k-user dataset. **Member surfaces** unified through `WBGam\Engine\MemberSurface` (+ `wb_gam_member_surface_html` filter): a BuddyPress profile "Achievements" tab (Overview/Badges/Points/Streak sub-tabs), a WooCommerce My Account `/my-account/achievements/` endpoint, and an opt-in LearnDash profile "My Achievements" link (`wb_gam_learndash_profile_link`, default off) — all reuse existing blocks + the mapped Hub page, no duplicated display logic. **Dark mode**: frontend surfaces map `--wb-gam-color-*` / `--gam-*` to the host theme's `--bx-color-*` tokens (BuddyX + BuddyX Pro light/dark), `color-mix` tints over the live surface, accent lightens on dark; themes without those tokens fall back to the original light palette. **Toast UX**: admin-set placement (Settings > Realtime; default bottom-right, corner-aware slide-in), every toast names what the points were for, SSE/Heartbeat dedupe fixed, no longer overlaps the theme header. **Jetonomy wiring**: `WBGam\Integrations\Jetonomy\DisplayDefer` defers the leaderboard to Jetonomy's reputation ranking when Jetonomy is active (reputation is already mirrored 1:1 into points, so the rankings are duplicates) — suppresses the `leaderboard` + `top-members` blocks/shortcodes and the Hub Leaderboard card; override with `wb_gam_defer_leaderboard_to_jetonomy`. Badges are deliberately kept (wb-gam's badge engine is the broader system; the two badge sets are complementary, not duplicates). **Fixes**: public profiles at `/u/{username}` now default on (were 404 for everyone); em-dashes removed from all translatable strings + seeded labels (hyphens only), existing DB badge descriptions migrated. **Inventory delta**: hooks_fired_filters 62→66 (added `wb_gam_defer_leaderboard_to_jetonomy`, `wb_gam_learndash_profile_link`, `wb_gam_member_surface_html`, `wb_gam_profile_publicly_visible`); regenerated openapi.json + sdk types. |
| **1.5.0** (2026-05-28) | **Second bug-sweep release. 21 Basecamp bug cards closed end-to-end** with real-data verification. 19 commits between `73f5db7` and current HEAD. **Two new local-CI gates added**: stage 2.13 `bin/check-boot-invariants.php` (catches `class_exists($name, false)` guards above a same-file class declaration — PHP class hoisting makes them always trigger, which silently disabled the admin menu in this session's first hour); stage 2.14 `bin/check-badge-condition-contract.php` (asserts every auto-awarded badge whose name/description promises a literal action is seeded as `action_count`, not `point_milestone`). **New engine**: `WBGam\Engine\MemberUploadCap` — bridges WP `upload_files` cap to logged-in members via `user_has_cap` filter so the achievement-editor Add Media button works for subscribers/contributors; opt-out filter `wb_gam_grant_member_uploads`. **Behaviour fixes**: WC purchase events now hook `woocommerce_payment_complete` (was `status_completed`, which never fires for paid-but-unshipped orders); 5 badge defaults switched from `point_milestone` to `action_count` (first_post, first_comment, prolific_writer, content_creator, engaged_reader); cohort tier names + new 5th-tier admin input wired through CohortEngine + REST + admin-cohort.js; CommunityChallengeEngine::award_community_bonus listener installed (the dead-letter that pre-fix queued AS jobs that no one handled); heartbeat interval → 5s (`fast`) for realtime toasts; toast.js dedupes by `_id` across footer/heartbeat/rest cursors; redemption email slug whitelisted in EmailSettingsController. **UX**: user-status-bar gets a proper chevron toggle + `--wb-gam-status-bar-top-offset` CSS var; cohort-rank tier-coloured accent dot (Bronze/Silver/Gold/Diamond/Obsidian); community-challenges completed-state visual treatment (green pill + 100% green bar + "Celebrating for X" copy) + per-block `style.css` now compiles; earning-guide card switched to two-row layout (label below icon+pts so long labels read horizontally); manual badge-award form on the Badge edit page; Hub Challenges card unites personal + community counts. **Boot**: the `class_exists` guard introduced in `06d811c` removed in `61f62ca` — it was the cause of the missing admin menu and the unfired setup wizard. **Inventory delta**: services 43 → 44 (MemberUploadCap added); structural counts (REST endpoints, AJAX, tables, blocks, shortcodes, admin pages, WP-CLI) unchanged at 56/0/23/19/17/13/10. |
