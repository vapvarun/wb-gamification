...
- [x] **Toast wrapper duplication — single-owner refactor** (2026-05-28). Killed the IA store's toast surface (`state.toasts`, `actions.dismissToast`, `ICON_FOR_TYPE`, `toastSequence`, the `default` switch branch in `callbacks.init`, the 6s auto-dismiss timer). Killed the duplicate `<div class="wb-gam-toasts">` template in `NotificationBridge::render()`. `toast.js` now owns the single container (dropped the `--rest` modifier — no CSS rule ever referenced it) and skips `level_up` + `streak_milestone` so those types render only as overlays. IA store gained a `wbGamRealtime` broker subscription so live heartbeat-delivered overlay events actually surface (previously the IA path only consumed the page-load seed; live deliveries skipped the overlay treatment — a real bug nobody noticed). Cross-path dedupe by `_id` keeps the overlay from re-opening when both the seed and the broker deliver the same event. Deleted `assets/js/toast.min.js` (pre-1.4.0 setInterval REST poller with emoji icons, never enqueued — pure dead weight from May 3). Browser-verified: single `.wb-gam-toasts` container in DOM, points event → toast, level_up → overlay only (no duplicate toast), streak → overlay only, 390px mobile readable.
...
- [x] **SSE / WebSocket transport** — All 4 stages shipped. Scaffold (stage 1), storage table + writer (stage 2, consolidated into `wb_gam_notifications_queue` from v2.2), streaming loop with all 6 environment hazards handled (stage 3), and browser-verified default flip to `'auto'` (stage 4 — see the SSE stage 4 entry above for the Playwright journey verification). WebSocket rejected as wrong tool (no bidirectional traffic; SSE wins on shared hosting + Cloudflare + auto-reconnect).
