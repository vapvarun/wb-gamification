...
The `wb_gam_points_for_action` filter lets you transform the points awarded for any action just before they're written to the ledger. No engine fork, no override — pure filter.
...
`wb_gam_points_for_action` runs INSIDE `PointsEngine::award_for_action` after:
- ✓ Rate limiter has approved the award
- ✓ `wb_gam_before_evaluate` filter has approved the event
...
- The `wb_gam_points_awarded` action fires

So your filter sees the final-pre-award value and gets the last word.

## Debugging

Hook `wb_gam_points_awarded` to log the FINAL value (after all your filters). The example file includes a debug-mode logger.
...
- `wb_gam_before_evaluate` — runs BEFORE points are computed; return modified event payload (or null to skip)
- `wb_gam_should_award_badge` — final veto on badge awards specifically (see [Example 05](../05-custom-badge-condition/))
- `wb_gam_event_metadata` — augment metadata before storage

## Don't do this

Don't use `wb_gam_points_for_action` to OVERRIDE manual admin awards — admin manual awards (via `POST /points/award` or `wp wb-gamification points award`) bypass this filter intentionally. They use `reason='manual_award'` and skip the rule pipeline. If you want to gate manual awards too, hook `wb_gam_before_points_awarded` instead (vetoable action, fires for both rule-driven and manual paths).

## Performance note

This filter fires on every event. Keep your callback fast — no DB queries per call if you can avoid it. The example file has a `get_posts()` lookup in the Tutorials-bonus filter; for production, cache the post→category mapping in the originating event payload via `wb_gam_event_metadata` so this filter doesn't have to re-query.
