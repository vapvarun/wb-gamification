...
( function () {
	...
	/**
	 * Wire one privacy toggle.
	 *
	 * This used to run straight down the file at parse time against a single
	 * `document.querySelector`, which meant it bound whatever was in the document AT THAT INSTANT and
	 * never looked again. On a host theme that navigates client-side, the member's own privacy control
	 * -- the one that decides whether strangers can see their profile -- arrived after the script had
	 * already given up, and the button did nothing at all.
	 *
	 * @param {Element} root The `.wb-gam-profile-privacy` element.
	 * @return {void}
	 */
	function bind( root ) {
		...
		var strings = {
			publicState:
				state.getAttribute( 'data-public-copy' ) ||
				'This profile is visible to anyone with the link.',
			privateState:
				state.getAttribute( 'data-private-copy' ) ||
				'Only you and site admins can see this profile.',
			makePublic:
				btn.getAttribute( 'data-make-public' ) || 'Make profile public',
			makePrivate:
				btn.getAttribute( 'data-make-private' ) ||
			...
			fetch( root.getAttribute( 'data-rest-url' ), {
				...
				.then( function ( result ) {
					if (
						result.ok &&
						result.data &&
						typeof result.data.public === 'boolean'
					) {
	...
	window.wbGam.onMount( '.wb-gam-profile-privacy', bind );
} )();
