...
- LearnDash profile: a single "My Achievements" link to the mapped Hub page (via the `learndash_shortcode_profile_before_template` hook, the one clean cover-area extension point), gated on LearnDash being active. Deliberately a link, not stat blocks - LearnDash has no native profile section API, so a single CTA reads cleanest.
