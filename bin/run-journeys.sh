#!/usr/bin/env bash
# bin/run-journeys.sh — orchestrate journey audits for WB Gamification.
#
...
while IFS= read -r -d '' f; do
  ...
  JOURNEYS+=("$rel")
done < <(find audit/journeys -type f -name '*.md' -not -name 'README.md' -not -name '.*' -not -name '_*' -print0 | sort -z)
